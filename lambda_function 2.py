...
def artist(data):
    #considering an instance where one song has multiple artists
    artist_list = []
    for row in data['items']:
        for key, value in row.items():
            if key == 'track':
                for artist in value['artists']:
                    artist_dict = {'artist_id': artist['id'], 'artist_ame': artist['name'], 'external_url': artist['href']}
...
def lambda_handler(event, context):
    ...
    Bucket = "spotify-etl-project-agodzo"
    ...
    for data in spotify_data:
        ...
        s3.put_object(Bucket=Bucket, Key=artist_key, Body=song_content)
        
     #copying data to processed folder from to_processed folder and deleting data in to_processed folder
